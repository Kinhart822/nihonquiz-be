...
describe('AdminController', () => {
  ...
  describe('Dashboard & Notifications', () => {
    it('should call getDashboardStats', async () => {
      adminService.getAdminDashboardStats.mockResolvedValue({
        totalStudents: 1,
      } as any);
      const result = await controller.getDashboardStats({ id: 1 } as any);
      expect(adminService.getAdminDashboardStats).toHaveBeenCalledWith(1);
      expect(result).toEqual({ totalStudents: 1 });
    });
